                    ...
                    <div key={`project-${projectId}`}>
                      ...
                      {!isCollapsed && (
                        <div className="ml-4 space-y-2 lg:space-y-3 xl:space-y-4 mb-4">
                          {projectTasks
                            .sort((a, b) => {
                              // Manter ordem por posição dentro do projeto
                              const posA = typeof a.position === 'number' ? a.position : 0;
                              const posB = typeof b.position === 'number' ? b.position : 0;
                              if (posA !== posB) {
                                return posA - posB;
                            ...
                            .map((task) => {
                            // Usar a posição real da task no array de todas as tasks da coluna
                            const realIndex = columnTasks.findIndex(t => t.id === task.id);
                            return (
                              <TaskCard 
                                key={task.id}
                                task={task}
                                index={realIndex >= 0 ? realIndex : task.position || 0}
                                tags={tags}
                                taskTags={taskTags}
                                projects={projects}
                                columns={columns}
